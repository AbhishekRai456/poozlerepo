...
int main(int argc, char* argv[]) {
    ...
    if (flag != "--exact" && flag != "--regex") {
        std::cerr << "Error: Invalid flag '" << flag << "'. Use --exact or --regex.\n";
        print_usage();
        return 1;
    }
    ...
    auto search_time = duration_cast<microseconds>(search_end - search_start).count();

    // 5. Output Results
    std::cout << "\n========================================\n";
    std::cout << "SEARCH RESULTS\n";
    std::cout << "========================================\n";
    std::cout << "Engine:      " << (flag == "--exact" ? "FM-Index (Exact)" : "Thompson NFA (Regex)") << "\n";

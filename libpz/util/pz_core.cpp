...
PzStd::PzCore::PzCore(PzBufferSPtr buffer) {
...
PzStd::PzBufferSPtr PzStd::PzCore::get_buffer() {
  return pz_buffer_sptr;
}
...
bool PzStd::PzCore::load_text(std::string_view text) {
  ...
  // We write text first so the regex engine can read it back via get_raw_text().
  // PzBuffer::load_text does NOT touch the `text` member — it only builds
  // the FM-Index and word list — so this assignment is not overwritten.
...
int PzStd::PzCore::count_exact(const std::string &pattern) {
  return pz_analysis_sptr->count(
      PzAnalysisType::PZ_ANALYSIS_TYPE_EXACT, pattern);
}

std::vector<int> PzStd::PzCore::locate_exact(const std::string &pattern) {
  return pz_analysis_sptr->locate(
      PzAnalysisType::PZ_ANALYSIS_TYPE_EXACT, pattern);
}
...
int PzStd::PzCore::count_regex(const std::string &pattern) {
  return pz_analysis_sptr->count(
      PzAnalysisType::PZ_ANALYSIS_TYPE_REGEX, pattern);
}
